...
public final class LoggerMyWay {
	...
	public static void configure(final int type, final String userName, final boolean debug) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Aplicatia nu poate initializa sistemul de loguri."
					+ "Este posibil sa nu aveti drept de scriere la adresa specificata. \nAplicatia se va inchide acum.", "Mesaj de eroare", JOptionPane.ERROR_MESSAGE);
	...
	private static void initProps(final Properties props, final int type) {
		/**
		 * setam proprietatile pentru RollingLog, un log care scrie in fisiere multiple, intr-o singura instanta, cand aceste fisiere ating dimensiunea maxima specificata.
		 */
		switch (type) {
			case LOG_TXT: {
	...
	/**
	 * not sure, but i guess something like that should be used when a hotswap (app running) is performed, in order to change the type of the current log file format.
	 */
